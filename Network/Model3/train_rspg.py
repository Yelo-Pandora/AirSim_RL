...
def train(args):
    ...
    collision_count = 0
    trapped_count = 0
    metrics_header_written = os.path.exists(config.TRAIN_METRICS_CSV) and os.path.getsize(config.TRAIN_METRICS_CSV) > 0
    ...
    for episode in range(start_episode, max_episodes):
        ...
        if (episode + 1) % config.TRAIN_LOG_INTERVAL == 0 or global_steps >= total_timestep_budget:
            reward_window = episode_rewards[-config.TRAIN_REWARD_WINDOW :]
            avg_reward = float(np.mean(reward_window)) if reward_window else 0.0
            success_rate = success_count / max(len(episode_rewards), 1) * 100.0
            collision_rate = collision_count / max(len(episode_rewards), 1) * 100.0
            trapped_rate = trapped_count / max(len(episode_rewards), 1) * 100.0
    ...
    print(f"Total global steps: {global_steps}")
    print(f"Overall success rate: {success_count / total_episodes_run * 100:.1f}%")
    print(f"Overall collision rate: {collision_count / total_episodes_run * 100:.1f}%")
    print(f"Overall trapped rate: {trapped_count / total_episodes_run * 100:.1f}%")
