...
class UAVRLOEnv:
    ...
    def step(self, action):
        ...
        local_target_obstacle_dist, _ = self.planner.voxel_grid.get_distance_with_obstacle(world_target)
        if local_target_obstacle_dist is None:
            local_target_obstacle_dist = config.RANGE_MAX
        ...
        if self.planner_mode == "ego":
            ...
            has_collision = False
            if self.use_airsim:
                self._execute_straight_airsim(world_target)
                if self._check_collision_airsim():
                    self.collision_count += 1
                    has_collision = True
            else:
        ...
        reward = compute_reward(
            ...
            self.prev_pos,
            self.goal,
            min_dist_to_obstacle=local_target_obstacle_dist,
            sigma=config.REWARD_SIGMA,
            beta=config.REWARD_BETA,
        )

        dist_to_goal = float(np.linalg.norm(self.current_pos - self.goal))
        ...
        info = {
            ...
            "local_target_obstacle_dist": float(local_target_obstacle_dist),
