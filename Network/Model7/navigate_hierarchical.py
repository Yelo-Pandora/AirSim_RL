...
from ppo_executor import PPOSegmentExecutor


def print_plan(plan):
    print(
        f"[Model7] Upper {plan.get('planner', 'csv')} A*: {len(plan['points'])} local targets, "
...
def main():
    parser = argparse.ArgumentParser(description="Model7 hierarchical A* + external PPO UAV navigation.")
    ...
    parser.add_argument("--ppo-model", type=str, default=None, help="External PPO checkpoint path")
    parser.add_argument(
        "--planner",
    ...
    if args.planner == "occupancy" or args.validate_waypoints:
        if args.plan_only:
            ...
            executor = PPOSegmentExecutor(model_path=args.ppo_model)
    ...
    print(f"[Model7] Finished {arrived_segments}/{len(plan['points']) - 1} segments.")
