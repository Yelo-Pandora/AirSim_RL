...
# 终点可视化配置
# ==========================
# 说明：这里使用 AirSim/Colosseum 的 debug marker，而不是生成实体物体。
# 这样更适合传感器验证脚本：不参与物理碰撞，也更容易自动过期。
SHOW_GOAL_MARKER = True
SHOW_GOAL_TEXT = False      # 不建议开永久文字，避免 GOAL 残留
SHOW_WAYPOINTS = True
GOAL_RADIUS = 1.5           # 与训练环境的 arrived 半径保持一致
GOAL_MARKER_DURATION = 2.0  # 非持久化标记持续时间，略大于一次循环/重置间隔
...
def to_airsim_vec(arr):
    """numpy/list -> AirSim Vector3r。AirSim 使用 NED 坐标，z 为负表示向上。"""
    return airsim.Vector3r(float(arr[0]), float(arr[1]), float(arr[2]))


def clear_visual_markers(client):
    """清除上一轮持久 debug marker，防止旧点、旧线残留。"""
    try:
        client.simFlushPersistentMarkers()
    except Exception as e:
        print(f"[Marker Warning] 清除可视化标记失败: {e}")


def build_circle_points(center, radius=GOAL_RADIUS, segments=64):
    """在 NED 坐标平面内生成终点判定圆。"""
    points = []
    for i in range(segments + 1):
        theta = 2.0 * math.pi * i / segments
        points.append(airsim.Vector3r(
...
def build_waypoints(start_pos, target, count=16):
    """生成和训练环境一致的等间距航点。"""
    start_pos = np.asarray(start_pos, dtype=np.float32)
    target = np.asarray(target, dtype=np.float32)
    return [start_pos + (target - start_pos) * (i / float(count)) for i in range(1, count + 1)]


def draw_goal_marker(client, start_pos=DEFAULT_START, target=DEFAULT_TARGET, duration=GOAL_MARKER_DURATION):
    """
    在 UE/Colosseum 场景中显示起点、终点、参考路径和终点判定圆。
    这里全部使用非持久化 marker，避免之前遇到的 GOAL 残留问题。
    """
    if not SHOW_GOAL_MARKER:
        return

    try:
        start = to_airsim_vec(start_pos)
        goal = to_airsim_vec(target)
        ...
        # 终点判定范围圆圈
        client.simPlotLineStrip(
            build_circle_points(target, radius=GOAL_RADIUS),
        ...
        # 16 个等间距航点，和训练环境保持一致
        if SHOW_WAYPOINTS:
            waypoint_points = [to_airsim_vec(wp) for wp in build_waypoints(start_pos, target, count=16)]
            client.simPlotPoints(
                waypoint_points,
        ...
        # 文字只使用短 duration，默认关闭，避免 GOAL 字样清不掉
        if SHOW_GOAL_TEXT:
            client.simPlotStrings(
                ["GOAL"],
                [airsim.Vector3r(float(target[0]), float(target[1]), float(target[2]) - 2.0)],
                scale=2.5,
                color_rgba=[1.0, 0.0, 0.0, 1.0],
                duration=duration
            )
...
def set_pose_and_hover(client, vehicle_name, start_pos=DEFAULT_START, target=DEFAULT_TARGET):
    # 先清一次，防止上一轮的持久化 debug marker 残留
    clear_visual_markers(client)
    ...
    # 飞机稳定后绘制当前验证用的终点标记
    draw_goal_marker(client, start_pos=start_pos, target=target, duration=GOAL_MARKER_DURATION)
...
def build_dashboard(scene, raw_gray, raw_color, zero_mask_vis, resized_vis, pooled_vis, lidar_strip, lidar_105_strip, lidar_report, block_text, action_summary, yaw_probe_summary, state, settings_summary, runtime_depth_size, capture_report, raw_stats):
    ...
    footer = "Keys: Q quit | R rerun probes | Goal marker: red point / yellow radius / blue reference line"
