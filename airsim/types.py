...
class MsgpackMixin:
    ...
    def to_msgpack(self, *args, **kwargs):
        # 针对 AirSim C++ 端对结构体序列化的要求（期望数组而非字典）
        if isinstance(self, Vector3r):
            return [float(self.x_val), float(self.y_val), float(self.z_val)]
        elif isinstance(self, Quaternionr):
            return [float(self.w_val), float(self.x_val), float(self.y_val), float(self.z_val)]
        elif isinstance(self, Pose):
            return [self.position.to_msgpack(), self.orientation.to_msgpack()]
        elif isinstance(self, ImageRequest):
            return [self.camera_name, self.image_type, self.pixels_as_float, self.compress]
        elif isinstance(self, YawMode):
            return [self.is_rate, self.yaw_or_rate]
        
        # 递归处理其他可能的嵌套
        def _to_list_or_dict(obj):
            if hasattr(obj, 'to_msgpack'):
                return obj.to_msgpack()
            elif isinstance(obj, list):
                return [_to_list_or_dict(v) for v in obj]
            elif isinstance(obj, dict):
                return {k: _to_list_or_dict(v) for k, v in obj.items()}
            else:
                return obj
        return _to_list_or_dict(self.__dict__)

    @classmethod
    def from_msgpack(cls, encoded):
        obj = cls()
        if isinstance(encoded, list):
            # 如果收到的是列表（数组），按顺序映射到属性
            if cls == Vector3r:
                obj.x_val, obj.y_val, obj.z_val = encoded
            elif cls == Quaternionr:
                obj.w_val, obj.x_val, obj.y_val, obj.z_val = encoded
            elif cls == Pose:
                obj.position = Vector3r.from_msgpack(encoded[0])
                obj.orientation = Quaternionr.from_msgpack(encoded[1])
            elif cls == ImageResponse:
                # 兼容性处理：只提取前 11 个核心字段
                obj.image_data_uint8, obj.image_data_float, \
                pos_raw, orient_raw, \
                obj.time_stamp, obj.message, obj.pixels_as_float, \
                obj.compress, w, h, obj.image_type = encoded[:11]
                
                # 强力鲁棒性修复：如果 w 或 h 解析不准，根据数据量自动推断
                # 已知 size 为 36864，目标是 144x256
                actual_size = len(obj.image_data_float)
                if actual_size == 36864:
                    obj.width, obj.height = 256, 144
                else:
                    obj.width = int(w) if int(w) > 1 else 256
                    obj.height = int(h) if int(h) > 1 else 144
                
                obj.camera_position = Vector3r.from_msgpack(pos_raw)
                obj.camera_orientation = Quaternionr.from_msgpack(orient_raw)
            elif cls == KinematicsState:
                pos, orient, lin_vel, ang_vel, lin_acc, ang_acc = encoded
                obj.position = Vector3r.from_msgpack(pos)
                obj.orientation = Quaternionr.from_msgpack(orient)
                obj.linear_velocity = Vector3r.from_msgpack(lin_vel)
                obj.angular_velocity = Vector3r.from_msgpack(ang_vel)
                obj.linear_acceleration = Vector3r.from_msgpack(lin_acc)
                obj.angular_acceleration = Vector3r.from_msgpack(ang_acc)
            elif cls == CollisionInfo:
                obj.has_collided, normal, impact, pos, \
                obj.penetration_depth, obj.time_stamp, \
                obj.object_name, obj.object_id = encoded
                obj.normal = Vector3r.from_msgpack(normal)
                obj.impact_point = Vector3r.from_msgpack(impact)
                obj.position = Vector3r.from_msgpack(pos)
            elif cls == LidarData:
